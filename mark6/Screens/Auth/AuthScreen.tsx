...
export default function AuthScreen() {
  ...
  const handleAuth = async () => {
    try {
      ...
      } else if (email == "dis@gmail.com") {
        await signInWithEmailAndPassword(auth, email, password);
        Alert.alert("Success", "Distributor logged in successfully!");
        navigation.reset({
          index: 0,
          routes: [{ name: "DistributorHome" }], // Navigate to the main screen after login
        });
      } else {
        await signInWithEmailAndPassword(auth, email, password);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.logoContainer}>
        <Image
          source={require("../../assets/hydroLogo.png")}
          style={styles.logo}
        />
      </View>
